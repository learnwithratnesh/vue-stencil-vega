...
if (!win.__stencil_cssshim && needsShim()) {
    ...
    var regex = /\/stencil-starter-project-name(\.esm)?\.js($|\?|#)/;
    var scriptElm = currentScript || Array.from(doc.querySelectorAll('script')).find(function(s) {
      return regex.test(s.src) || s.getAttribute('data-stencil-namespace') === "stencil-starter-project-name";
    });

    var resourcesUrl = scriptElm ? scriptElm.getAttribute('data-resources-url') || scriptElm.src : '';
    var start = function() {
      var url = new URL('./p-6ffab44d.system.js', resourcesUrl);

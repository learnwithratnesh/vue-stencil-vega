import { BUILD, NAMESPACE } from '@stencil/core/internal/client/build-conditionals';
...
const patchEsm = () => {
    ...
    if (BUILD.cssVarShim && !(win.CSS && win.CSS.supports && win.CSS.supports('color', 'var(--c)'))) {
        // @ts-ignore
        return import('./css-shim.stencil-client.mjs').then(() => {
...
const patchBrowser = () => {
    ...
    if (importMeta !== '') {
        ...
        if (!window.customElements) {
            ...
            // @ts-ignore
            return import('./dom.stencil-client.mjs').then(() => opts);
        }
    }
    ...
    if ((BUILD.lazyLoad || BUILD.style) && (hostRef.$flags$ & 32 /* hasInitializedComponent */) === 0) {
        ...
        if (BUILD.style && !styles.has(scopeId) && Cstr.style) {
            ...
            if (!BUILD.hydrateServerSide && BUILD.shadowDom && cmpMeta.$flags$ & 8 /* needsShadowDomShim */) {
                style = await import('./shadow-css.stencil-client.mjs').then(m => m.scopeCss(style, scopeId, false));
            }
...
export { Build, Context, H, H as HTMLElement, Host, STENCIL_DEV_MODE, attachShadow, bootstrapLazy, connectedCallback, consoleDevError, consoleDevInfo, consoleDevWarn, consoleError, createEvent, defineCustomElement, disconnectedCallback, doc, forceUpdate, getAssetPath, getConnect, getContext, getElement, getHostRef, getMode, getRenderingElement, getValue, h, insertVdomAnnotations, isMemberInElement, loadModule, moduleCache, nextTick, parsePropertyValue, patchBrowser, patchCloneNodeFix, patchDynamicImport, patchEsm, plt, postUpdateComponent, proxyComponent, proxyCustomElement, proxyNative, readTask, registerHost, registerInstance, renderVdom, setMode, setValue, styles, supportsConstructibleStylesheets, supportsListenerOptions, supportsShadowDom, win, writeTask };
